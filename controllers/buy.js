...
module.exports.getBuy = function(req,res){//Fetch
	...
	if(req.query.user_id){
		...
		Buy.count(query,function(err_count,res_count){
			...
			Buy.find(query).sort({"index_count":-1}).skip(parseInt(skip)).limit(parseInt(limit))
	...
	UserSubMap.find(query_sub,function(err_sub, result_sub){
		if(result_sub.length>0){
			...
			if(toDateObj>currentDateObj && parseInt(result_sub[0].remain_post) > 0){
				//var buy_req_id = "1";
				Counter.getNextSequenceValue('buy',function(sequence){
					if(sequence){
				...
				{	
					if(maxValue.length && maxValue.length > 0){
						...
						index_count = (buy_req_id - (- (maxValue[0].buy_req_id).substr(7)));
					}
					else{
						buy_req_id = "BUYREQ_1";
					}*/
					...
					doc.buy_req_id = "BUYREQ_"+index_count;//buy_req_id;
					doc.createdAt = d;
					doc.changedAt = d;
...
module.exports.deleteBuy = function(req,res){//Delete
	Buy.remove({buy_req_id: req.params.id}, function(post_err,post_result){
		if(post_err){
